...
const buttonVariants = cva(
  'inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-md text-sm font-medium transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:size-4 [&_svg]:shrink-0 touch-manipulation select-none',
  {
    variants: {
      variant: {
      ...
      size: {
        default: 'h-10 sm:h-9 px-4 py-2 min-h-[44px] sm:min-h-[36px]',
        sm: 'h-9 sm:h-8 rounded-md px-3 text-xs min-h-[44px] sm:min-h-[32px]',
        lg: 'h-12 sm:h-10 rounded-md px-8 min-h-[48px] sm:min-h-[40px]',
        icon: 'h-10 w-10 sm:h-9 sm:w-9 min-h-[44px] min-w-[44px] sm:min-h-[36px] sm:min-w-[36px]',
      },
    },
    defaultVariants: {
      variant: 'default',
